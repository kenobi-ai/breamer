...
console.log("Is this Cloud Run????", isCloudRun);
...
app.get("/health", (req, res) => {
  res
    .status(200)
    .json({ status: "ok", service: "breamer", socketio: "enabled" });
});
...
(global as any).activeSockets = activeSockets;

const ensureSessionBootstrapped = async (
  clientId: string,
  session: any,
  socket: Socket
) => {
  if (!session || !session.requiresBootstrap) {
    return;
  }

  session.cdpSession?.removeAllListeners?.("Page.screencastFrame");

  const viewportWidth = session.viewport?.width ?? 1440;
  const viewportHeight = session.viewport?.height ?? 1880;

  await browserManager.initializeScreencast(
    session.cdpSession,
    viewportWidth,
    viewportHeight
  );
  ...
  socket.emit("session_ready", {
    ...
    recovered: Boolean(session.wasRecovered),
  });

  session.wasRecovered = false;
  session.requiresBootstrap = false;
};
...
io.on("connection", (socket) => {
  ...
  console.log(
    `[Socket.io] Client connected: ${clientId} (Active: ${io.engine.clientsCount})`
  );
  console.log(
    `[Sessions] Current browser sessions: ${Array.from(
      browserManager.getSessions().keys()
    ).join(", ")}`
  );
  ...
  setTimeout(async () => {
    try {
      ...
      if (existingSession) {
        console.log(
          `[Socket.io] Reusing existing browser session for ${clientId}`
        );
        await ensureSessionBootstrapped(clientId, existingSession, socket);
        return;
      }
  ...
  socket.on("hover", async (data) => {
    try {
      ...
      await ensureSessionBootstrapped(clientId, session, socket);
      await ResilientMessageHandlers.handleHover(
        session.page,
        socket,
        data.x,
        data.y
  ...
  socket.on("set_viewport", async (data) => {
    try {
      ...
      console.log(
        `[Viewport] Setting viewport for ${clientId} to ${data.width}x${data.height}`
      );
  ...
  socket.on("disconnect", async (reason) => {
    console.log(
      `[Socket.io] Client disconnected: ${clientId}, reason: ${reason} (Active: ${
    ...
    console.log(
      `[Sessions] Remaining browser sessions: ${Array.from(
        browserManager.getSessions().keys()
      ).join(", ")}`
    );
  });
...
httpServer.listen(PORT, HOST, () => {
  ...
  console.log(
    `🔄 Transports: ${
      isCloudRun ? "websocket-only (Cloud Run mode)" : "polling → websocket"
    }`
  );
  console.log(`☁️  Environment: ${process.env.NODE_ENV || "development"}`);
  console.log(`🔐 Trust proxy: enabled`);
  console.log(
    `🏃 Running on: ${isCloudRun ? "Google Cloud Run" : "Standard deployment"}`
  );

  if (isCloudRun) {
    console.log(
      "⚠️  Cloud Run detected - using WebSocket-only mode to avoid session affinity issues"
    );
  }
});
